...
xtable(EDA, display = c("s","s","d","d", "d","f", "d", "d", "f", "f", "d", "f"), 
       label = "ta:viaBaits", 
...
the exploratory data analysis on bait to prey interaction datasets. 
This compendium serves as a supplementary source of materials for the
analysis derived in \emph{Coverage and Error Models of Protein-Protein
Interaction Data by Directed Graph Analysis}.
\end{abstract}

\section{Introduction}
This is a technical report that demonstrates the use of \Rpackage{ppiStats}, 
\Rpackage{ppiData}, and \Rpackage{yeastExptData} R packages. 
It accompanies the paper \emph{Coverage and Error Models of Protein-Protein 
Interaction Data by Directed Graph Analysis} by Chiang et al. This report explains 
all the steps taken to perform the analysis on the protein interaction data.
This text has been produced as a reproducible document. It contains 
...
of R (version 2.4.1 or greater) together with recent versions of the three packages 
listed above and of some other add-on packages. 
To reproduce the computations shown here, you do not need to type 
them or copy-paste them from the PDF file; rather, you can take the file 
supp.Rnw in the doc directory of the \Rpackage{ppiStats} package, open it in a 
text editor, run 
it using the R command Sweave, and modify it to your needs. Alternatively, if
you would simply like the code without the surrounding text, you can call 
...
\cite{Krogan2006} dataset and rendered its vertex induced subgraph in
...
The interaction data between four
selected proteins from Krogan et al.'s experiment~\cite{Krogan2006}. The
bi-directional edge between the ATPase \textit{SSA1} and the translational 
elongation factor \textit{TEF2}
indicates that either one as a bait pulled down the other one as a
prey. The directed edge from \textit{RPC82}, a subunit of RNA polymerase III, 
to \textit{SSA1} indicates that \textit{RPC82} as a bait pulled down 
\textit{SSA1}, but not vice versa. 
Another unreciprocated edge goes from
the phosphatase \textit{PHO3} to\textit{TEF2}.  An investigation of the dataset
shows that \textit{PHO3}, which localizes in the periplasmatic space, 
was not reported in any interaction as a prey, while \textit{RPC82C} was.
In the interpretation of the data, we would have most confidence that there is a 
real interaction between \textit{SSA1} and \textit{TEF2}. We can differentiate between
the two unreciprocated interactions: the one
between \textit{RPC82C} and \textit{SSA1} has been bi-directionally
tested, but only found once, while the other one has only been uni-directionally 
tested and found.}
\end{figure}
...
proportion of the yeast interactome tested by each 
...
@ 
     \begin{figure}
       ...
       \caption{
         \label{fig:piecharts}{
           The number of proteins probed in 
           different experiments. Colored in light blue, the viable baits (VB) 
           are those proteins
           tested as baits which found at least one other interacting 
           partner. Colored Green, the viable prey (VP) are those proteins tested as
           prey which were found by a viable bait. Viable bait/prey
           (VBP) are proteins which are simultaneously VB and VP and colored in
           dark blue. The remaining proteins (grey) were either not tested 
           or failed to show an interaction with any other protein
           in the experiment. }
       }
     \end{figure}
...
@ 
       ...
       \caption{
         \label{fig:caiplots}{
           ...
           derived from the \cite{Gavin2006} experimental data set.
           ...
           rows randomly permuted (and the columns sorted accordingly) while the
           left panel has ordered the proteins according to their CAI value. 
           The patterns in the left panel indicate an association 
           between CAI and the number of bait to prey interactions. In fact, 
           we can determine that the relationship is based on proteins with 
...
<<zScoreHist, fig=TRUE, width=18, height=18, echo=FALSE, results=hide, include=FALSE>>=

par(mfrow = c(3,4), cex.main = 2, cex.lab=1.5, cex.axis=1.5)
mapply(function(x,y) {
  hist(x, main=y, col="blue")},
  zScore1, dNames1)
@ 
...
<<zScoreDens, fig=TRUE, width=18, height=18, prefix=TRUE, echo=FALSE, results=hide, include=FALSE>>=
par(mfrow = c(3,1), cex.main = 2, cex.lab=1.5, cex.axis=1.5)
zScore2 <- zScore1[c(1,10,11)]
dNames2 <- dNames1[c(1,10,11)]
m2 <- m[c(1,10,11)]
dens <- lapply(zScore2, density)
sh <- lapply(zScore2, shorth)
mapply(function(x,y,z,w) {plot.density(x, main=paste("Distribution of the z-scores for ", y, sep=""), col="blue");
                      abline(v=z, col="red");
                          abline(v=w, col="green")}, dens, dNames2, sh, m2)
...
their densities (Figure~\ref{fig-zScoreDens}). With more data points, we can 
...
\myincfig{matrixPlot}{\textwidth}{Adjacency matrix representing
the tested interactions of two separate experiments. The light 
blue square shows the set of tested interactions for experiment
1 while the light green shows the tested interactions for 
experiment 2. The dark blue square indicates those tested
interactions common to both experiments while the gray shaded
area were untested in both.}

As an example we show how sampling is fundamental for 
inter-experimental analysis. 

The
possible pitfalls of naive comparisons between two experimental
