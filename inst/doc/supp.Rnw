...
makeBPMat <- function(){
  ...
  for(g in bpExperimentNames) {
    ...
    if(nrow(m)>1) {
      stopifnot(identical(test[[g]],vbp))
...
\newpage\myincfig{fig-pfppfnFiltSep}{\textwidth}{The solution manifolds
...
stats[,7] <- round(stats[,4]/stats[,6], digits=2)
stats[,8] <- sapply(as.list(bpRed)[names(vbp)], function(x) {sum(x*t(x))/2})
stats[,9] <- round(stats[,5]/stats[,8], digits=2)

rownames(stats) <- dNames1
colnames(stats) <- letters[1:9]

cap <- "A statistical analysis on the expected number of positiv errors on the filtered data. 
        Column a: number of proteins retained after the filtering the data;
        Column b: the approximate number of protein pairs which do not interact;
        Column c: \pfp given that \pfn is zero;
        Column d: the expected number of unreciprocated false positive observations;
        Column e: the expected number of reciprocated false positive observations;
        Column f: the actual number of observed unreciprocated interactions;
        Column g: the false discovery rate for unreciprocated observations;
        Column h: the actual number of observed recriprocated interactions;
        Column i: the false discovery rate of observed reciprocated interactions."

stats
@ 

<<pfnStats, echo=FALSE, results=hide>>=
stats2 <- matrix(0, 11, 9)
...
stats2[,7] <- round(stats2[,4]/stats2[,6], digit=2)
stats2[,8] <- sapply(as.list(bpRed)[names(vbp)], 
                                      function(x) {sum(!(x*t(x)))/2})
stats2[,9] <- round(stats2[,5]/stats2[,8], digit=2)
rownames(stats2) <- dNames1
colnames(stats2) <- letters[1:9]
stats2
cap2 <- "A statistical analysis on the expected number of negative errors on the filtered data. 
        Column a: estimated number of interactions given \pfp is zero;
        Column b: \pfn given that \pfp is zero;
        Column c: the expected number of unreciprocated false negative observations;
        Column d: the false discovery rate for unreciprocated observations;
        Column e: the expected number of reciprocated false negative observations;
        
        Column f: the false discovery rate of observed reciprocated non-adjacent protein pairs."
@ 

<<pfpTab, echo=FALSE, results=tex>>=
xtable(stats, display=c("s","d","d","f","d","d","d","f","d", "f"), label="ta:pfp", 
       tabular.environment="longtable", floating=FALSE, size="small",
       caption=cap)
...
xtable(stats2, display=c("s","d","d","f","d","d","d","f","d", "f"), label="ta:pfn", 
       tabular.environment="longtable", floating=FALSE, size="small", caption=cap2)
