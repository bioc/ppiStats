...
Even though the hypergeometric tests show that there are 
\Sexpr{length(pvalues(hg1)[pvalues(hg1)<hg1@pvalueCutoff])} number
of GO terms which are over-represented by the viable baits of 
\textit{Ito-Core} dataset, we have decided only to investigate 
those GO terms which at least 50 annotated genes.
