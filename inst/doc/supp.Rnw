...
This copendium serves as a supplementary source of materials for the
analysis derived in \emph{A Directed Graph Analysis of Protein Interaction
Data}.
\end{abstract}
...
interaction datasets. We wanted data that had two properties: 1. the 
bait/prey relationship is preserved and 2. that the prey population 
is documented as genome-wide. These properties are necessary for 
the directed graph analysis. Under these conditions, we downloaded the 
protein interaction data of \cite{Ito2001, Uetz2000, Zhao2005, Cagney2001, 
...
data, we created the R-data package \Rpackage{ppiData} where each dataset
is stored as a directed graph (digraph).
...
(ORFs) corresponding to each protein. If the ORF is
...
cache(vbpStochastic <- lapply(vbpGraph, function(x) 
                              {if(length(nodes(x))!=0) 
...
representation within Gene Ontology. Using the R software packages
\Rpackage{Category} and \Rpackage{GOstats}, we were able
to asses these questions. The code used to test for over and
under representation can be found in the \Robject{hgGO.R} script and is a 
separate entity. It has been bundled with the supplementary materials 
but can also be found within the inst/Scripts/ directory of the R packge 
\Rpackage{ppiStats}. Running the script will generate .html files 
of those GO categories which are either over/under represented. 
For our purposes, we used a p-value cutoff threshold of $10^{-2}$. 
In addition, we were only interested in those GO categories which
contained at least 50 unique annotations. Both these parameters can
be set by the user, and those who are familiar with the R programming
language are free to manipulate these parameters within the R scripts.
...
of data for each point on in each of the figures. 
...
\begin{itemize}
  \item Number of proteins affected by a two-sided binomial test.
  \item Number of proteins affected by a one-sided binomial test where
...
The Fisher tests show relatively small p-values for all three
$2 \times 2$ tables though the most significant occurs for the 
one-sided binomial test when the in-degree dominates the 
out-degree. 
...
$n_{\rho} = i_{\rho}+o_{\rho}$, we can, without loss of generality, assume 
that an out-edge represents a success in our binomial model. Therefore, 
we can standardize this directed degree and compute the \emph{z-score} for 
each protein:
...
R \Rfunction{density} function, we were able to render the density distribution 
for each dataset.
...
<<zScoreDens, fig=TRUE, width=18, height=18, echo=FALSE, results=hide>>=
...
mapply(function(x,y,z,w) {
  plot.density(x, 
               main=y, col="blue");
  abline(v=z, col="red");
  abline(v=w, col="green")}, dens, dNames1, sh, m)
@ 

The Density plots of the z-score distributions are given for eleven datasets.
The green lines corresponds to the mean of the distributions while the red 
lines corrsponds to the \emph{shorth} of the distribution. The \emph{shorth}
first calculates a minimal bin such that at approximately 50\% of the mass 
falls within this bin and then calculates the median for this bin. In essence,
a modified interquartile range is computed along with the modified median. This
can describle a tightly clusterd set of data points by their median value.
