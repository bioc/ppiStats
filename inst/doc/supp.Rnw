...
mean measure of abundance of a yeast cell in a SD medium (Table~\ref{ta:cor}). The Spearman 
correlation seems to be the more accurate measure, not simply because 
it is larger, but because the relationship between CAI and abundance is not
linear \cite{Sharp1987}. 
...
We plotted the values of each protein's CAI value against $(\log)$ the three sets of
measured abundance data to visulize this association (Figure~\ref{fig-plotCor}). 
The most interesting 
fact is that the measured abundance has the highest correlation with CAI. This 
seems to suggest that the reference set of genes used to compute all CAI
might be highly expressed under SD medium.
