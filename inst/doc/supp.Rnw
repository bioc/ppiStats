...
(IntAct accension code, SwissProt ID, etc) is used. 
...
In addition to the Logistic Regressions, we plotted the adjacency matrix diagram of 
of the bait/prey interactions in two different ways: 1. the rows and columns 
randomly ordered and 2. the rows and columns ordered by ascending CAI. 
...
distributions. We can see that for datasets such as 
