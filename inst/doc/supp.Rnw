...
testing on the GO directed acyclic graphs (DAG). The file
...
abundance has the highest correlation with CAI. This seems to suggest that 
the reference set of genes used to compute all CAI might be highly expressed 
under SD medium.
