...
(cf Figures~\ref{fig-plotCorGav, fig-plotCorKro}). 
...
proteins affected by a bias in both experiments (${|S_1 \cap S_2|}_X$); in 
$(1,2)$-entry, we count the number affected in \cite{Gavin2006} only 
(${|S_1 \setminus S_2|}_X$); in $(2,1)$, the number in \cite{Krogan2006} only
(${|S_2 \setminus S_1|}_X$); and in $(1,1)$, the number not 
affected in both $({|S_1^c \cap S_2^c|_X})$. We can create three separate 
...
cache(dG <- calcInOutDegStats(vbpGraph[["Gavin2006BPGraph"]]))
cache(dK <- calcInOutDegStats(vbpGraph[["Krogan2006BPGraph"]]))
...
and see if we such artefacts are reproducible across experiments.
...
test, we can describe experiment-wide artefacts by using the same binomial model 
for each protein. 
...
stats2[,3] <- c(.76, 0.39, 0.3, .55, .65, .55, .44, .68, .37, .44, .8)
...
FP estimates, unreciprocated FN errors are estimated by $\pfn(1-\pfn)n$; 
