...
with the supplementary materials but can also be found in 
the R packge \Rpackage{ppiStats}.
...
$\Theta\equiv(\ntot, \delta^*, \pfp, \pfn)$, this is a 2D
matrix with infinite numbers of rows and columns, corresponding to
$n_{R}$ and $n_{U}$. However, most of the
probability mass is concentrated within a bounded range.  Furthermore,
...
This is computed by the following function \Rfunction{assessSymmetry},
given a bait/prey interaction (adjacency) matrix \Robject{m}. In addition,
...
a reduced set of data \Robject{bpRed} that contains only proteins with
$p$-values larger than or equal to p-value threshold of $10^{-2}$.
%
...
\[
n_{i} ~|~ n_{u} ~~~ \sim \mathcal{B}\left( n_{i} + n_{o}, 
  \frac{1}{2} \right)
\]
...
\item Use test outcomes as responses to fit logistic regressions with
  abundance as predictor \footnote{We actually use logarithm (base 2)
    of abundance as predictor since that has a much more symmetric
    distribution.}.
\end{itemize}
...
universe is VBP$_{C} = $ VBP$_{\mbs{Gavin2006}} \cap$ VBP$_{\mbs{Krogan2006}}$.
\cite{Gavin2006} index the rows; \cite{Krogan2006}, the columns. 
Each entry of the table is given by $(i,j) : i,j \in \{1,2\}$. 
...
The binomial model derived in ~\ref{sec:inout} allows us to determine
...
we can, without loss of generality, assume that an in-edge represents a 
...
%--------------------------------------------------
The following part is taken from the vignette \textit{solve\_pfp\_pfn.Rnw} 
that WH wrote in July 2006. Please excuse the slight inconsistency of 
notation, for now. Given
\par\vspace*{1ex}
\begin{tabular}{rp{0.25\textwidth}p{0.65\textwidth}}
&$\frac{1}{2}\ntot(\ntot-1)$&the total number of interactions (excluding homomers)\\
$n_1$&&the true number of interactions\\
$n_2$&$=\frac{1}{2}\ntot(\ntot-1)-n_1$&the true number of non-interactions\\
$X_1$&&observed number of reciprocated edges\\
$X_2$&&observed number of non-edges\\
$X_3$&$=n_1+n_2-X_1-X_2$&observed number of unreciprocated eges
\end{tabular}
\par\vspace*{1ex}
\noindent we have
\begin{eqnarray}
E[X_1] &=& n_1\,(1-\pfn)^2+n_2\,\pfp^2\label{x1}\\
E[X_2] &=& n_1\,\pfn^2+n_2\,(1-\pfp)^2\label{x2}\\
E[X_3] &=& 2n_1\,\pfn(1-\pfn)+2n_2\,\pfp(1-\pfp)\label{x3}
\end{eqnarray}
...
$x_2$, $x_3$. Since we do not know $n_1$, the above system of two
independent equations for three variables defines a one-dimensional
solution manifold, and the goal of this vignette is to investigate
that solution manifold.

We will parameterize that manifold by $n_1$ ($0\le n_1\le b$) in
$(\pfp,\pfn)$-space. Relevant solutions are those for which 
$0\le \pfp,\pfn \le 1$.

Consider that $n_1$ is given. Let us solve 
Equations~\eqref{x1}--\eqref{x3} for $\pfp$ and $\pfn$. First,
subtracting $\eqref{x4} = \eqref{x1} - \eqref{x2}$, we have
\begin{eqnarray}
x_1-x_2&=&n_1\,(1-2\pfn)-n_2\,(1-2\pfp)\label{x4}\\
\Leftrightarrow\quad
\pfn&=&\frac{1}{2n_1}\left((x_2-n_2)-(x_1-n_1)+2n_2\,\pfp\right)\nonumber\\
\pfn&=&\frac{1}{2n_1}\left(\Delta+2n_2\,\pfp\right),\label{q}
\end{eqnarray}
where we have defined $\Delta:= (x_2-n_2)-(x_1-n_1)$ for convenience.
We can plug this expression for $\pfn$ into Equation~\eqref{x2} and obtain
\begin{equation}
\underbrace{\left(n_1+n_2\right)}_{=:a} \pfp^2 + 
\underbrace{\left(\Delta-2n_1\right)}_{=:b} \pfp + 
\underbrace{n_1+\frac{\Delta^2}{4n_2}-\frac{n_1}{n_2}x_2}_{=:c} = 0.
\end{equation}
The equation $ax^2 + bx +c=0$ is solved by
\begin{equation}
x_{1,2}=\frac{-b\pm\sqrt{b^2-4ac}}{2a}.\label{p}
\end{equation}
Hence, the problem is solved: for data $\ntot$, $x_1$, $x_2$
...
values of $n_1$ will lead to admissible solutions for $\pfp$ and 
...
\texttt{ntot} ($\ntot$) need to be scalars, \texttt{n1} ($n_1$) can be a vector.
% 
<<simulate>>=
...
<<pfppfnUnfiltSep, echo=TRUE, fig=TRUE, eps=FALSE,include=FALSE,width=8.5,height=12, results=hide>>=
par(mfrow=c(4,3))
...
The result of this is plotted in Figure~\ref{fig-pfppfnUnfiltSep}. 
\myincfig{fig-pfppfnUnfiltSep}{\textwidth}{The solution manifolds
  (one plot per dataset, unfiltered data).}


<<pfppfnAll2gether, echo=TRUE, fig=TRUE, eps=FALSE,include=FALSE,width=10,height=15, results=hide>>=
if(!exists("pfppfnAll2gether")){
...
@
%
The result of this is plotted in Figure~\ref{fig-pfppfnAllTogether}. 
