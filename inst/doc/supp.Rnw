...
or another identifier (IntAct accession code, SwissProt ID, etc) 
is used. 
...
contains published data on protein abundance.
...
sampling statistics on the Y2H and APMS datasets on a per experiment
...
{ 
    ...
    glm(formula, ginfo, family = family, control = glm.control(maxit = 100) ,...)  
}
...
\caption{
         \label{fig:caiplots1}{
           Same plots for the Krogan et al's~\cite{Krogan2006}
           data.
           }
       }
     \end{figure}
...
sets of measured abundance data to visulize this association 
(Figure~\ref{fig-plotCor}). The most interesting fact is that the measured 
protein abundances in SD medium have the highest correlation with CAI. This 
seems to suggest that the reference set of genes used to compute all CAI might 
be highly expressed under SD medium.
...
universe is $X$ =  VBP$_{\cite{Gavin2006}} \mbox{ } \cap$ 
VBP$_{\cite{Krogan2006}}$.
...
cache(kSG <- subGraph(x, get("Krogan2006BPGraph")))
cache(gSG <- subGraph(x, get("Gavin2006BPGraph")))
g1 <- idStochastic(kSG, bpGraph=TRUE)
k1 <- idStochastic(gSG, bpGraph=TRUE)
g2 <- setdiff(x, g1)
k2 <- setdiff(x, k1)

cache(dsG <- calcInOutDegStats(gSG))
cache(dsK <- calcInOutDegStats(kSG))
...
The previous tables used each's experiments VBP subgraph to find proteins which
are suspected of having a systematic bias. We then restricted to $X$ to calculate
two way tables based on these proteins. In the following tables, we first 
restrict to the node induced subgraph induced by $X$ for each experiment, and then 
seach for proteins affected by systematic bias. We then create two way tables 
based on these proteins and see whether or not these sets are independent.
...
the two-way table \Robject{tab2Way} and the results from its
...
proteins tested in \Robject{tab1WayIN}), we 
see an exceptionally small p-value, and so we should probably
reject the null hypothesis that these two sets of systematic
proteins are independent. For those proteins which out-degree
dominates in-degree (cf \Robject{tab1WayOUT}), we see a less 
significant p-value, and so 
the association between these sets of proteins is not as strong
as the prior sets. For the sake of completeness, all the tests
show relatively small p-values as well as substantial odds 
ratios.
...
For three plots that appear to have either a strong positive
or negative concentration of data points in the distributions, we plotted 
...
which this comparison is insightful. First we found the set theoretic intersection
of the two datasets, i.e. $X = VBP_{\cite{Gavin2006}}\cap VBP_{\cite{Krogan2006}}$, and
from this new set of proteins, we plot the $z$-score computed from the data of 
\cite{Gavin2006} against the $z$-score computed from the data of \cite{Krogan2006}
(See Figure~\ref{fig-zScoreInt}). From the plot, we were also able to generate
correlation coefficients between the $z$-scores \cite{Gavin2006} and the 
...
stats[,3] <-  c(.0008, .04, .07, .015, .003, .015, .004, .005, 
                .012, .0015, .0015)
stats[,4] <- round(stats[,2]*stats[,3]*(1-stats[,3]))
...
stats2 <- matrix(0, 11, 7)
stats2[,1] <- sapply(as.list(bpRed)[names(vbp)], nrow)
stats2[,2] <- c(1200, 8, 2, 20, 78, 34, 584, 649, 223, 2429, 11744)
stats2[,3] <- c(.78, 0.4, 0.3, .55, .62, .56, .41, .62, .38, .41, .8)
stats2[,4] <- round(stats2[,2]*stats2[,3]*(1-stats2[,3]))
...
on the protein interactions, and let $N$ be the number of proteins in the filtered
...
Unreciprocated FP errors can be estimated by $\pfp(1-\pfp)m$; reciprocated,
...
number of unreciprocated and reciprocated false negative observations. We 
...
FP estimates, unreciprocated FN errors is estimated by $\pfn(1-\pfn)n$; 
...
commProtsY2H <- matrix(0, nrow = 7, ncol=7)
dimnames(commProtsY2H) <- list(dNames[1:7], dNames[1:7])
for(i in 1:7){
    for (j in i:7){
        commProtsY2H[i,j] <- length(intersect(viableBaits[[i]], viableBaits[[j]]))

    }    

    for(k in 1:i){
        commProtsY2H[i,k] <- length(intersect(viablePrey[[i]], viablePrey[[k]]))
    }
}
...
xtable(commProtsY2H, display=c("s", "d", "d", "d", "d", "d", "d", "d"),
