...
\item[] \textbf{Bait:} 
  Any protein sampled for the purposes of ascertaining the 
  proteins with which it interacts. 
  The set of baits used in an experiment is the bait population. 
\item[] \textbf{Cloned Bait:} 
  Any bait that was successfully cloned in a yeast cell with either
  a binding domain (Y2H) or a specified tag (AP-MS). 
\item[] \textbf{Viable Bait:} 
  Any cloned bait that was observed to detect one or more proteins (prey). 
\item[] \textbf{Prey:} 
  Any protein that is tested against the bait proteins. The set of prey used 
...
\item[] \textbf{Viable Prey:} 
  Any prey that was found to interact with a viable bait. 
